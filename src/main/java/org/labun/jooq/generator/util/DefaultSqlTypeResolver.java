...
/**
 * Most of code in this class was originally taken from org.jooq.codegen project just as is and need
 * to be cleaned and rethinked.
 *
...
public class DefaultSqlTypeResolver implements TypeResolver {
  ...
  static Name getArrayBaseType(SQLDialect dialect, String t, Name u) {
    ...
    throw new SQLDialectNotSupportedException(
        "getArrayBaseType() is not supported for dialect " + dialect);
  }

  @Override
  public String resolve(SubGeneratorContext ctx, DataTypeDefinition def) {
    return getJavaTypeReference(ctx, def.getDatabase(), def);
  }

  protected String getTypeReference(SubGeneratorContext ctx, Database db, SchemaDefinition schema,
      String t,
      int p, int s, int l, boolean n, String d, Name u) {
    ...
    // [#5334] In MySQL, the user type is (ab)used for synthetic enum types. This can lead to accidental matches here
      ...
      sb.append(DefaultDataType
          .normalise(DefaultDataType.getDataType(db.getDialect(), String.class).getTypeName()));
      ...
      try {
        dataType = mapJavaTimeTypes(ctx, DefaultDataType.getDataType(db.getDialect(), t, p, s))
            .nullable(n);
      ...
      // Otherwise, reference the dialect-specific DataType itself.
        ...
        try {
          ...
          if (!type1.equals(type2)) {
            Class<?> clazz = mapJavaTimeTypes(ctx,
                DefaultDataType.getDataType(db.getDialect(), t, p, s)).getType();
  ...
  private DataType<?> mapJavaTimeTypes(SubGeneratorContext ctx, DataType<?> dataType) {
    DataType<?> result = dataType;

    // [#4429] [#5713] This LOGic should be implemented in Configuration
    if (dataType.isDateTime() && ctx.getConfig().getJavaTimeDates()) {
  ...
  protected String getJavaTypeReference(SubGeneratorContext ctx, Database db,
      DataTypeDefinition type) {

    // [#4388] TODO: Improve array handling
    if (db.isArrayType(type.getType())) {
      Name baseType = getArrayBaseType(db.getDialect(), type.getType(),
  ...
  @Deprecated
  protected String getType(SubGeneratorContext ctx, Database db, SchemaDefinition schema, String t,
      int p,
      int s, String u, String javaType, String defaultType) {
    return getType(ctx, db, schema, t, p, s, name(u), javaType, defaultType);
  }

  protected String getType(SubGeneratorContext ctx, Database db, SchemaDefinition schema, String t,
      int p,
      int s, Name u, String javaType, String defaultType) {
    return getType(ctx, db, schema, t, p, s, u, javaType, defaultType,
  ...
  protected String getType(SubGeneratorContext ctx, Database db, SchemaDefinition schema, String t,
      int p,
      int s, String u, String javaType, String defaultType, GeneratorStrategy.Mode udtMode) {
    return getType(ctx, db, schema, t, p, s, name(u), javaType, defaultType, udtMode);
  }

  protected String getType(SubGeneratorContext ctx, Database db, SchemaDefinition schema, String t,
      int p,
      int s, Name u, String javaType, String defaultType, GeneratorStrategy.Mode udtMode) {
    ...
    // Array types
      ...
      type =
          getType(ctx, db, schema, baseType.last(), p, s, baseType, javaType, defaultType, udtMode)
              + "[]";
    }
    ...
    // Check for Oracle-style VARRAY types
      ...
      if (udtMode == GeneratorStrategy.Mode.POJO || (udtMode == GeneratorStrategy.Mode.INTERFACE
          && !udtArray)) {
        type =
            "java.util.List<" + getJavaType(ctx, db.getArray(schema, u).getElementType(), udtMode)
                + ">";
      } else if (udtMode == GeneratorStrategy.Mode.INTERFACE) {
        type =
            "java.util.List<? extends " + getJavaType(ctx, db.getArray(schema, u).getElementType(),
                udtMode) + ">";
      } else {
        type = getStrategy()
            .getFullJavaClassName(db.getArray(schema, u), GeneratorStrategy.Mode.RECORD);
      }
    }
  ...
  protected String getJavaType(SubGeneratorContext ctx, DataTypeDefinition type) {
