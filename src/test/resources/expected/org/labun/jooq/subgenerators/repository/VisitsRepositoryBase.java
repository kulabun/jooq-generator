...
public class VisitsRepositoryBase {
    protected final org.labun.jooq.subgenerators.table.VisitsTable table = org.labun.jooq.subgenerators.table.VisitsTable.VISITS;
    protected final DSLContext jooq;
    private final Map<String, Field> pkFieldsMap;

    public VisitsRepositoryBase(DSLContext dslContext) {
        this.jooq = dslContext;
        this.pkFieldsMap = getPKFieldsMap();
    }

    public boolean isFor(Table table) {
        return table != null && org.labun.jooq.subgenerators.table.VisitsTable.class.isAssignableFrom(table.getClass());
    }
    ...
    public int delete(Iterable<Object> entities) {
        ...
        List<org.labun.jooq.subgenerators.record.VisitsRecord> records = StreamSupport.stream(entities.spliterator(), false)
                .map(it -> toRecord(it))
    ...
    public <T> T save(T entity) {
        org.labun.jooq.subgenerators.record.VisitsRecord record = toRecord(entity);
        record.store();
    ...
    private org.labun.jooq.subgenerators.record.VisitsRecord toRecord(Object entity) {
        Map<Field, Object> id = getId(entity);
        if (id.values().stream().anyMatch(it -> it == null))
    ...
    private org.labun.jooq.subgenerators.record.VisitsRecord updateRecord(Map<Field, Object> id, Object entity) {
        return fetchLockedById(id)
                .map(it -> applyFrom(it, entity))
                .orElse(null);
    }

    private org.labun.jooq.subgenerators.record.VisitsRecord applyFrom(org.labun.jooq.subgenerators.record.VisitsRecord row, Object update) {
        row.from(update);
        return row;
    }

    private org.labun.jooq.subgenerators.record.VisitsRecord insertRecord(Object entity) {
    ...
    private <T> List<T> applyRecordsToEntities(Iterable<T> entities, List<org.labun.jooq.subgenerators.record.VisitsRecord> records) {
        List<T> result = new ArrayList<>();
        Iterator<T> entitiesIterator = entities.iterator();
        Iterator<org.labun.jooq.subgenerators.record.VisitsRecord> recordsIterator = records.iterator();
        while (entitiesIterator.hasNext() && recordsIterator.hasNext()) {
            org.labun.jooq.subgenerators.record.VisitsRecord record = recordsIterator.next();
    ...
    private List<TableField<org.labun.jooq.subgenerators.record.VisitsRecord, ?>> pk() {
        UniqueKey<org.labun.jooq.subgenerators.record.VisitsRecord> key = table.getPrimaryKey();
        if (key == null) throw new IllegalStateException("no pk found");
        return key.getFields();
    }

    protected Optional<org.labun.jooq.subgenerators.record.VisitsRecord> fetchById(Map<Field, Object> id) {
    ...
    protected Optional<org.labun.jooq.subgenerators.record.VisitsRecord> fetchLockedById(Map<Field, Object> id) {
        return buildCondition(id)
                .map(condition ->
                        jooq.selectFrom(table)
                                .where(condition)
