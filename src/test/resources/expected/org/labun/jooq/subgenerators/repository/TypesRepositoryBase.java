...
public class TypesRepositoryBase {
    protected final org.labun.jooq.subgenerators.table.TypesTable table = org.labun.jooq.subgenerators.table.TypesTable.TYPES;
    protected final DSLContext jooq;
    private final Map<String, Field> pkFieldsMap;

    public TypesRepositoryBase(DSLContext dslContext) {
        this.jooq = dslContext;
        this.pkFieldsMap = getPKFieldsMap();
    }

    public boolean isFor(Table table) {
        return table != null && org.labun.jooq.subgenerators.table.TypesTable.class.isAssignableFrom(table.getClass());
    }
    ...
    public int delete(Iterable<Object> entities) {
        ...
        List<org.labun.jooq.subgenerators.record.TypesRecord> records = StreamSupport.stream(entities.spliterator(), false)
                .map(it -> toRecord(it))
    ...
    public <T> T save(T entity) {
        org.labun.jooq.subgenerators.record.TypesRecord record = toRecord(entity);
        record.store();
    ...
    private org.labun.jooq.subgenerators.record.TypesRecord toRecord(Object entity) {
        Map<Field, Object> id = getId(entity);
        if (id.values().stream().anyMatch(it -> it == null))
    ...
    private org.labun.jooq.subgenerators.record.TypesRecord updateRecord(Map<Field, Object> id, Object entity) {
        return fetchLockedById(id)
                .map(it -> applyFrom(it, entity))
                .orElse(null);
    }

    private org.labun.jooq.subgenerators.record.TypesRecord applyFrom(org.labun.jooq.subgenerators.record.TypesRecord row, Object update) {
        row.from(update);
        return row;
    }

    private org.labun.jooq.subgenerators.record.TypesRecord insertRecord(Object entity) {
    ...
    private <T> List<T> applyRecordsToEntities(Iterable<T> entities, List<org.labun.jooq.subgenerators.record.TypesRecord> records) {
        List<T> result = new ArrayList<>();
        Iterator<T> entitiesIterator = entities.iterator();
        Iterator<org.labun.jooq.subgenerators.record.TypesRecord> recordsIterator = records.iterator();
        while (entitiesIterator.hasNext() && recordsIterator.hasNext()) {
            org.labun.jooq.subgenerators.record.TypesRecord record = recordsIterator.next();
    ...
    private List<TableField<org.labun.jooq.subgenerators.record.TypesRecord, ?>> pk() {
        UniqueKey<org.labun.jooq.subgenerators.record.TypesRecord> key = table.getPrimaryKey();
        if (key == null) throw new IllegalStateException("no pk found");
        return key.getFields();
    }

    protected Optional<org.labun.jooq.subgenerators.record.TypesRecord> fetchById(Map<Field, Object> id) {
    ...
    protected Optional<org.labun.jooq.subgenerators.record.TypesRecord> fetchLockedById(Map<Field, Object> id) {
        return buildCondition(id)
                .map(condition ->
                        jooq.selectFrom(table)
                                .where(condition)
