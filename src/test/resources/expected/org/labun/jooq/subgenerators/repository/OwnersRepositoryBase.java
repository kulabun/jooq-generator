...
public class OwnersRepositoryBase {
    protected final org.labun.jooq.subgenerators.table.OwnersTable table = org.labun.jooq.subgenerators.table.OwnersTable.OWNERS;
    protected final DSLContext jooq;
    private final Map<String, Field> pkFieldsMap;

    public OwnersRepositoryBase(DSLContext dslContext) {
        this.jooq = dslContext;
        this.pkFieldsMap = getPKFieldsMap();
    }

    public boolean isFor(Table table) {
        return table != null && org.labun.jooq.subgenerators.table.OwnersTable.class.isAssignableFrom(table.getClass());
    }
    ...
    public int delete(Iterable<Object> entities) {
        ...
        List<org.labun.jooq.subgenerators.record.OwnersRecord> records = StreamSupport.stream(entities.spliterator(), false)
                .map(it -> toRecord(it))
    ...
    public <T> T save(T entity) {
        org.labun.jooq.subgenerators.record.OwnersRecord record = toRecord(entity);
        record.store();
    ...
    private org.labun.jooq.subgenerators.record.OwnersRecord toRecord(Object entity) {
        Map<Field, Object> id = getId(entity);
        if (id.values().stream().anyMatch(it -> it == null))
    ...
    private org.labun.jooq.subgenerators.record.OwnersRecord updateRecord(Map<Field, Object> id, Object entity) {
        return fetchLockedById(id)
                .map(it -> applyFrom(it, entity))
                .orElse(null);
    }

    private org.labun.jooq.subgenerators.record.OwnersRecord applyFrom(org.labun.jooq.subgenerators.record.OwnersRecord row, Object update) {
        row.from(update);
        return row;
    }

    private org.labun.jooq.subgenerators.record.OwnersRecord insertRecord(Object entity) {
    ...
    private <T> List<T> applyRecordsToEntities(Iterable<T> entities, List<org.labun.jooq.subgenerators.record.OwnersRecord> records) {
        List<T> result = new ArrayList<>();
        Iterator<T> entitiesIterator = entities.iterator();
        Iterator<org.labun.jooq.subgenerators.record.OwnersRecord> recordsIterator = records.iterator();
        while (entitiesIterator.hasNext() && recordsIterator.hasNext()) {
            org.labun.jooq.subgenerators.record.OwnersRecord record = recordsIterator.next();
    ...
    private List<TableField<org.labun.jooq.subgenerators.record.OwnersRecord, ?>> pk() {
        UniqueKey<org.labun.jooq.subgenerators.record.OwnersRecord> key = table.getPrimaryKey();
        if (key == null) throw new IllegalStateException("no pk found");
        return key.getFields();
    }

    protected Optional<org.labun.jooq.subgenerators.record.OwnersRecord> fetchById(Map<Field, Object> id) {
    ...
    protected Optional<org.labun.jooq.subgenerators.record.OwnersRecord> fetchLockedById(Map<Field, Object> id) {
        return buildCondition(id)
                .map(condition ->
                        jooq.selectFrom(table)
                                .where(condition)
